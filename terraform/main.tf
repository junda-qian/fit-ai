...
# S3 bucket for health documents (PDFs)
...
resource "aws_apigatewayv2_route" "post_calculate_energy" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "POST /calculate-energy"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
...
resource "aws_apigatewayv2_route" "post_generate_workout_plan" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "POST /generate-workout-plan"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
