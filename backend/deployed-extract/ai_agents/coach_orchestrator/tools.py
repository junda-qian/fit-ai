...
def _get_database():
    """Get database instance - supports both local JSON and AWS DynamoDB"""
    import sys
    import os

    # Check if running in AWS Lambda
    if os.environ.get('AWS_LAMBDA_FUNCTION_NAME'):
        # Return a DynamoDB wrapper
        import boto3
        ...
        class DynamoDBWrapper:
            def __init__(self):
                ...
                self.table_prefix = os.environ.get('TABLE_PREFIX', 'health-chatbot-dev')

            def find_one(self, table_name, query):
                """Find one item matching query"""
                try:
                    table = self.dynamodb.Table(f'{self.table_prefix}-{table_name}')
                    response = table.get_item(Key=query)
                    item = response.get('Item')
                    return convert_decimals(item) if item else None
                except Exception as e:
                    print(f"DynamoDB find_one error: {e}")
                    return None
            ...
            def find(self, table_name, query):
                ...
                try:
                    table = self.dynamodb.Table(f'{self.table_prefix}-{table_name}')
                    # For nutrition_logs and workout_logs, scan with filter
                    if 'user_id' in query:
                        response = table.scan(
                            FilterExpression='user_id = :uid',
                            ExpressionAttributeValues={':uid': query['user_id']}
                        )
...
def get_nutrition_recommendation(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use the DynamoDB-aware database wrapper
        db = _get_database()
